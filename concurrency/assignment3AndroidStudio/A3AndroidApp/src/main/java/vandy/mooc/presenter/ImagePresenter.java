...
import vandy.mooc.DownloadAsyncTask;
import vandy.mooc.FilterAsyncTask;
...
public class ImagePresenter 
    ...
    public void startProcessing() {
        if (mUrlList.isEmpty())
            ...
            {
                DownloadAsyncTask dat = new DownloadAsyncTask(getApplicationContext());
                FilterAsyncTask fat = new FilterAsyncTask(getApplicationContext());
                dat.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR, url, mDirectoryPathname);
                try {
                    if(dat.get() == null)
                    {
                        onProcessingComplete(url, null);
                    }
                    else {
                        fat.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR, dat.get(), mDirectoryPathname);
                        onProcessingComplete(url, fat.get());
                    }
